...
use serde::{
    de::{
        ...
        EnumAccess, Error as DeserializeError, IntoDeserializer, VariantAccess, Visitor,
    },
    Deserializer as _,
};
use thiserror::Error;
...
impl<'de: 'a, 'a> serde::Deserializer<'de> for Deserializer {
    ...
    {
        match self.value {
            Value::String(v) => visitor.visit_enum(v.borrow_ref()?.clone().into_deserializer()),
            Value::StaticString(v) => visitor.visit_enum(v.to_string().into_deserializer()),
