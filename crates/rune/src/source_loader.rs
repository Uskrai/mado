...
impl SourceLoader {
        ...
        loop {
            match item.pop_front() {
                ...
                Some(Component::Id(..) | Component::Crate(..)) => {
                    return Err(CompileErrorKind::UnsupportedModuleItem { item: base_item })
                }
                // break when empty
                None => {
                    break;
    ...
    fn search_module(mut root: PathBuf, name: &str) -> Result<PathBuf, CompileErrorKind> {
        if root.is_file() {
...
mod test {
    ...
    pub fn create_environment(list_path: &[&str]) -> Result<TempDir, Box<dyn std::error::Error>> {
        let dir = tempfile::tempdir()?;
        let path = dir.path();
        for it in list_path {
            std::fs::create_dir_all(path.join(it).parent().unwrap())?;
