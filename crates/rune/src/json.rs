...
trait JsonMethod {
    ...
    fn select_as_value(&'static self, query: &str) -> Result<Vec<Value>, crate::Error> {
    ...
    /// Return json as [Self]
    fn select_as_json(&'static self, query: &str) -> Result<Vec<JsonRef>, crate::Error> {
        let selected = jsonpath_lib::select(self.inner(), query)?;
...
trait JsonMethodMut: JsonMethod {
    ...
    fn convert_to_rune_mut(value: &'static mut JValue) -> Value {
        ...
        match value {
            JValue::Object(_) => Value::Any(Shared::new(AnyObj::new(JsonRef::new(value)))),
            JValue::Array(v) => {
                let val = v.iter_mut();
...
pub fn load_module() -> Result<rune::compile::Module, rune::compile::ContextError> {
