...
impl<'a> Build<'a> {
    ...
    #[inline(always)]
    pub fn source_loader(mut self, source_loader: &'a mut dyn RuneSourceLoader) -> Self {
    ...
    #[inline(always)]
    pub fn with_path(self, path: &Path) -> Result<SourceBuild<'a>, std::io::Error> {
        Ok(self.with_source(Source::from_path(path)?))
    }

    #[inline(always)]
    fn build_unit_diagnostics(self, sources: &mut Sources) -> Result<Unit, Diagnostics> {
    ...
    pub fn build_unit(self, mut sources: Sources) -> Result<Unit, BuildError> {
        self.build_unit_diagnostics(&mut sources)
    ...
    #[inline(always)]
    pub fn build_for_module(self, sources: Sources) -> Result<ModuleBuild, BuildError> {
