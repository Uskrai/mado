...
use rune::{
    runtime::{Args, GuardedArgs, RuntimeContext, VmError as RuneVmError, VmSendExecution},
...
impl Rune {
    pub fn new(context: Arc<Context>, unit: Arc<Unit>, sources: Arc<Sources>) -> Self {
    ...
    pub fn send_execute<N, A>(&self, name: N, args: A) -> Result<VmSendExecution, VmError>
    where
        N: IntoTypeHash,
        A: Send + Args,
    {
    ...
    pub fn convert_result<T>(&self, result: Result<T, RuneVmError>) -> Result<T, VmError> {
        match result {
            Ok(value) => Ok(value),
    ...
    pub async fn async_call<R, N, A>(&self, name: N, args: A) -> Result<R, VmError>
    where
        N: IntoTypeHash,
        A: GuardedArgs,
        R: FromValue,
